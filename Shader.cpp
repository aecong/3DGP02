...
void CObjectsShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext)
{
	...
	for (int i = 0; i < m_nObjects; i++)
	{
		...
		m_ppObjects[i]->SetMovingSpeed(5.f);
...
void CBOShader::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	{
		CB_GAMEOBJECT_INFO* pbMappedcbGameObject = (CB_GAMEOBJECT_INFO*)((UINT8*)m_pcbMappedGameObjects + (j * ncbElementBytes));
		XMStoreFloat4x4(&pbMappedcbGameObject->m_xmf4x4World, XMMatrixTranspose(XMLoadFloat4x4(&m_ppObjects[j]->m_xmf4x4World))); // world를 transform을 바꾼다면?
	}
}
